...
class DataPreprocessingBeforeClassifiers:
    ...
    def fit_tokenizers(self, df_train_path: str, df_test_path: str):
        ...
        self.df_train['title'] = self.df_train['title'].apply(lambda x: [word for word in x if word not in self.stop_words])
        self.df_train['text'] = self.df_train['text'].apply(lambda x: [word for word in x if word not in self.stop_words])
        self.df_test['title'] = self.df_test['title'].apply(lambda x: [word for word in x if word not in self.stop_words])
        self.df_test['text'] = self.df_test['text'].apply(lambda x: [word for word in x if word not in self.stop_words])

        self.df_train['title'] = self.df_train['title'].apply(lambda x: [self.lemmatizer.lemmatize(token) for token in x])
        self.df_train['text'] = self.df_train['text'].apply(lambda x: [self.lemmatizer.lemmatize(token) for token in x])
        self.df_test['title'] = self.df_test['title'].apply(lambda x: [self.lemmatizer.lemmatize(token) for token in x])
        self.df_test['text'] = self.df_test['text'].apply(lambda x: [self.lemmatizer.lemmatize(token) for token in x])
    ...
    def transform_ml(self, text: str) -> np.array: # Use this for inference of single examples
        tokenized = self._tokenize_lemmatize_text(text)
        transformed = self.ml_vectorizer.transform(tokenized)
        return transformed
    
    def transform_dl(self, text: str) -> np.array: # Use this for inference of single examples
        tokenized = self._tokenize_lemmatize_text(text)
